module rof_comp_nuopc
  ...
  use RtmVar                , only : rtmlon, rtmlat, ice_runoff, iulog, nt_hydros
  use RtmVar                , only : nsrStartup, nsrContinue, nsrBranch
  use RtmVar                , only : hydros_active, flood_active
...
!$  call omp_set_num_threads(nthrds)
    ...
    if (masterproc) then
       ...
       write(iulog,*) ' hydros npes = ',npes
       write(iulog,*) ' hydros iam  = ',iam
    ...
    if (hydros_active) then
       allocate (totrunin(runoff%begr:runoff%endr,nt_hydros))
    end if
  ...
  subroutine ModelAdvance(gcomp, rc)
    ...
    if(masterproc) then
       lbnum=1
       call memmon_dump_fort('memmon.out','hydros_comp_nuopc_ModelAdvance:start::',lbnum)
    endif
#endif
!$  call omp_set_num_threads(nthrds)
    ...
    !--------------------------------
    ! First advance hydros time step
    !--------------------------------
    ...
    call t_startf ('lc_hydros_import')
    call import_fields(gcomp, totrunin, rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call t_stopf ('lc_hydros_import')

    !--------------------------------
    ! Run RTM
    !--------------------------------

    ! Advance hydros time step (export data is in runoff and Trunoff data types)
   ...
   !call get_curr_date( yr, mon, day, tod, offset=-dtime ) TODO: WHY DOES THIS NOT WORK?
    ...
    if ( (ymd /= ymd_sync) .or. (tod /= tod_sync) ) then
       write(iulog,*)'  hydros ymd=',ymd     ,'  hydros tod= ',tod
    ...
    if(masterproc) then
       lbnum=1
       call memmon_dump_fort('memmon.out','hydros_comp_nuopc_ModelAdvance:end::',lbnum)
       call memmon_reset_addr()
    endif
#endif
  ...
  subroutine ModelFinalize(gcomp, rc)
    ...
    ! local variables
    character(*), parameter :: F00   = "('(hydros_comp_nuopc) ',8a)"
    character(*), parameter :: F91   = "('(hydros_comp_nuopc) ',73('-'))"

...
! in MCT (Model Coupling Toolkit) format and converting it to use by RTM
  ...
  use RtmVar           , only : rtmlon, rtmlat, ice_runoff, iulog, &
                                ...
                                hydros_active, flood_active, nt_hydros, hydros_tracers
...
!========================================================================
  ...
  subroutine rof_init_mct( EClock, cdata_r, x2r_r, r2x_r, NLFilename)
    ...
    if (hydros_active) then
       ...
       begr = runoff%begr
       endr = runoff%endr
       allocate (totrunin(begr:endr,nt_hydros))
    ...
    ! Fill in infodata
    call seq_infodata_PutData( infodata, rof_present=hydros_active, &
         rofice_present=.false., rof_prognostic=hydros_active, &

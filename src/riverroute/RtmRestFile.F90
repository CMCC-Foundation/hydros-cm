...
! !USES:
  ...
  use RtmVar        , only : rtmlon, rtmlat, iulog, inst_suffix, rpntfil, &
                             ...
                             finidat_hydros, nrevsn_hydros, spval, &
...
!-----------------------------------------------------------------------
  ...
  subroutine RtmRestGetfile( file, path )
    ...
    ! Branch run:
    ! Restart file pathname is obtained from namelist "nrevsn_hydros"
    if (nsrest==nsrBranch) then
       length = len_trim(nrevsn_hydros)
       if (nrevsn_hydros(length-2:length) == '.nc') then
          path = trim(nrevsn_hydros)
       else
          path = trim(nrevsn_hydros) // '.nc'
       end if
       call getfil( path, file, 0 )

       ! Check case name consistency (case name must be different
       ! for branch run, unless brnch_retain_casename is set)
       ctest = 'xx.'//trim(caseid)//'.hydros'
       ftest = 'xx.'//trim(file)
    ...
    if (nsrest==nsrStartup) then
       call getfil( finidat_hydros, file, 0 )
    end if
...
!-----------------------------------------------------------------------
  ...
  subroutine restFile_read_pfile( pnamer )
    ...
    ! [nrevsn_hydros] contains the full pathname of the restart file.
...
!-----------------------------------------------------------------------
  ...
  character(len=256) function RtmRestFileName( rdate )
    ...
    RtmRestFileName = "./"//trim(caseid)//".hydros"//trim(inst_suffix)//".r."//trim(rdate)//".nc"
    if (masterproc) then
...
!-----------------------------------------------------------------------
  ...
  subroutine RtmRestart(ncid, flag)
    ...
    do nt = 1,nt_hydros
       ...
       if (nv == 1) then
          vname = 'RTM_VOLR_'//trim(hydros_tracers(nt))
          lname = 'water volume in cell (volr)'
          uname = 'm3'
          dfld  => runoff%volr(:,nt)
       elseif (nv == 2) then
          vname = 'RTM_FLUXOUT_'//trim(hydros_tracers(nt))
          lname = 'water fluxout in cell (fluxout)'
          uname = 'm3/s'
          dfld  => runoff%fluxout(:,nt)
       elseif (nv == 3) then
          vname = 'RTM_RUNOFF_'//trim(hydros_tracers(nt))
          lname = 'runoff (runoff)'
          uname = 'm3/s'
          dfld  => runoff%runoff(:,nt)
       elseif (nv == 4) then
          vname = 'RTM_DVOLRDT_'//trim(hydros_tracers(nt))

module RtmVar
  ...
  !TODO - nt_hydros and hydros_tracers need to be removed and set by access to the index array
  integer, parameter, public :: nt_hydros = 2    ! number of tracers
  character(len=3), parameter, public :: hydros_tracers(nt_hydros) =  (/'LIQ','ICE'/)
...
!================================================================================
  ...
  subroutine RtmVarInit( )
    if (masterproc) then
       ...
       if (nsrest == nsrBranch .and. nrevsn_hydros == ' ') then
       ...
       if (nsrest == nsrContinue) then
          nrevsn_hydros = 'set by restart pointer file file'
       end if

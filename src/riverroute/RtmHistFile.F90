...
!
  integer :: ni
  integer, public :: &
       hydroshist_ndens(max_tapes) = 1         ! namelist: output density of netcdf history files
  integer, public :: &
       hydroshist_mfilt(max_tapes) = 30        ! namelist: number of time samples per tape
  integer, public :: &
       hydroshist_nhtfrq(max_tapes) = (/0, -24, -24/)  ! namelist: history write freq(0=monthly)
  character(len=1), public :: &
       hydroshist_avgflag_pertape(max_tapes) = (/(' ',ni=1,max_tapes)/)   ! namelist: per tape averaging flag
  
  ! list of fields to add
  character(len=max_namlen+2), public :: hydroshist_fincl1(max_flds) = ' '
  character(len=max_namlen+2), public :: hydroshist_fincl2(max_flds) = ' '
  character(len=max_namlen+2), public :: hydroshist_fincl3(max_flds) = ' '
  !
  ...
  character(len=max_namlen+2), public :: hydroshist_fexcl1(max_flds) = ' '
  character(len=max_namlen+2), public :: hydroshist_fexcl2(max_flds) = ' '
  character(len=max_namlen+2), public :: hydroshist_fexcl3(max_flds) = ' '
...
!-----------------------------------------------------------------------
  ...
  subroutine RtmHistHtapesBuild ()
    ...
    character(len=1) :: avgflag    ! lcl equiv of hydroshist_avgflag_pertape(t)
    ...
    do t=1,max_tapes
       if (hydroshist_avgflag_pertape(t) /= ' ') then
          avgflag = hydroshist_avgflag_pertape(t)
          do f = 1,nfmaster
             select case (avgflag)
             case ('A')
    ...
    fincl(:,1) = hydroshist_fincl1(:)
    fincl(:,2) = hydroshist_fincl2(:)
    fincl(:,3) = hydroshist_fincl3(:)

    fexcl(:,1) = hydroshist_fexcl1(:)
    fexcl(:,2) = hydroshist_fexcl2(:)
    fexcl(:,3) = hydroshist_fexcl3(:)
    ...
    do t=1,ntapes
       tape(t)%ntimes = 0
       tape(t)%nhtfrq = hydroshist_nhtfrq(t)
       tape(t)%mfilt = hydroshist_mfilt(t)
       if (hydroshist_ndens(t) == 1) then
...
!-----------------------------------------------------------------------
  ...
  subroutine htapes_fieldlist()
    ...
    if (masterproc) then
       ...
       do t=1,ntapes
          write(iulog,*)
          if (hydroshist_nhtfrq(t) == 0) then
             write(iulog,*)'RTM History tape ',t,' write frequency is MONTHLY'
          else
             write(iulog,*)'RTM History tape ',t,' write frequency = ',hydroshist_nhtfrq(t)
          endif
          write(iulog,*)'Number of time samples on RTM history tape ',t,' is ',hydroshist_mfilt(t)
          write(iulog,*)'Output precision on RTM history tape ',t,'=',hydroshist_ndens(t)
          write(iulog,*)
       end do
       call shr_sys_flush(iulog)
    end if
...
!-----------------------------------------------------------------------
  ...
  subroutine htape_addfld (t, f, avgflag)
    ...
    integer :: numhydros          ! total number of hydros cells on all procs
...
!-----------------------------------------------------------------------
  ...
  subroutine htape_create (t, histrest)
    ...
    integer :: sec_hist_nhtfrq     ! hydroshist_nhtfrq converted to seconds
    ...
    !
    sec_hist_nhtfrq = hydroshist_nhtfrq(t)
    if (hydroshist_nhtfrq(t) < 0) then !hour need to convert to seconds
        sec_hist_nhtfrq = abs(hydroshist_nhtfrq(t))*3600
    end if
...
999 format(a,i0)
    ...
    call ncd_defdim( lnfid, 'allrof', numhydros    , dimid)
       ...
       if (tape(t)%is_endhist) then
          ...
          if (tape(t)%ntimes == 1) then
             locfnh(t) = set_hist_filename (hist_freq=tape(t)%nhtfrq, &
                                            hydroshist_mfilt=tape(t)%mfilt, hist_file=t)
             if (masterproc) then
  ...
  subroutine RtmHistRestart (ncid, flag, rdate)
    ...
    !================================================
       ...
       do t = 1,ntapes
          ...
          !
          write(hnum,'(i1.1)') t-1
          locfnhr(t) = "./" // trim(caseid) //".hydros"// trim(inst_suffix) &
    ...
    !================================================
       ...
       hydroshist_fincl1(:) = fincl(:,1)
       hydroshist_fincl2(:) = fincl(:,2)
       hydroshist_fincl3(:) = fincl(:,3)

       hydroshist_fexcl1(:) = fexcl(:,1)
       hydroshist_fexcl2(:) = fexcl(:,2)
       hydroshist_fexcl3(:) = fexcl(:,3)
...
!-----------------------------------------------------------------------
  ...
  character(len=max_length_filename) function set_hist_filename (hist_freq, hydroshist_mfilt, hist_file)
    ...
    integer, intent(in)  :: hydroshist_mfilt  !history file number of time-samples
    ...
    if (hist_freq == 0 .and. hydroshist_mfilt == 1) then   !monthly
    ...
    set_hist_filename = "./"//trim(caseid)//".hydros"//trim(inst_suffix)//&

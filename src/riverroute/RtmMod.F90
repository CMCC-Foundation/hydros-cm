...
! !USES:
  ...
  use RtmHistFile     , only : RtmHistUpdateHbuf, RtmHistHtapesWrapup, RtmHistHtapesBuild, &
                               ...
                               hydroshist_avgflag_pertape, hydroshist_avgflag_pertape, & 
...
! RTM tracers
  character(len=256) :: hydros_trstr   ! tracer string

! RTM naemlists
  integer :: hydros_tstep                    ! RTM time step

! RTM constants
  real(r8),save :: delt_hydros_max              ! RTM max timestep
...
! global rtm grid
  ...
  character(len=256) :: hydros_mode
  character(len=256) :: hydros_effvel    
...
!EOP
    ...
    integer ,allocatable :: num_hydros(:)        ! num of cells on each pe
    integer  :: nbas                          ! number of basins
    integer  :: nhydros                          ! num of rtm points
    integer  :: baspe                         ! pe with min number of rtm cells
    integer  :: maxhydros                        ! max num of rtms per pe for decomp
...
!-----------------------------------------------------------------------
    ...
    if (masterproc) then
       ...
       unitn = getavu()
       write(iulog,*) 'Read in hydros_inparm namelist from: ', trim(nlfilename_rof)
       open( unitn, file=trim(nlfilename_rof), status='old' )
       ier = 1
       do while ( ier /= 0 )
          read(unitn, hydros_inparm, iostat=ier)
          if (ier < 0) then
             call shr_sys_abort( subname//' encountered end-of-file on hydros_inparm read' )
          endif
       end do
       call relavu( unitn )
   end if

    call mpi_bcast (hydros_tstep,   1, MPI_INTEGER, 0, mpicom_rof, ier)
    ...
    call mpi_bcast (hydros_effvel,   len(hydros_effvel) , MPI_CHARACTER, 0, mpicom_rof, ier)

    call mpi_bcast (ice_runoff,  1, MPI_LOGICAL, 0, mpicom_rof, ier)

    call mpi_bcast (hydroshist_nhtfrq, size(hydroshist_nhtfrq), MPI_INTEGER,   0, mpicom_rof, ier)
    call mpi_bcast (hydroshist_mfilt , size(hydroshist_mfilt) , MPI_INTEGER,   0, mpicom_rof, ier)
    call mpi_bcast (hydroshist_ndens , size(hydroshist_ndens) , MPI_INTEGER,   0, mpicom_rof, ier)

    call mpi_bcast (hydroshist_fexcl1, (max_namlen+2)*size(hydroshist_fexcl1), MPI_CHARACTER, 0, mpicom_rof, ier)
    call mpi_bcast (hydroshist_fexcl2, (max_namlen+2)*size(hydroshist_fexcl2), MPI_CHARACTER, 0, mpicom_rof, ier)
    call mpi_bcast (hydroshist_fexcl3, (max_namlen+2)*size(hydroshist_fexcl3), MPI_CHARACTER, 0, mpicom_rof, ier)
    call mpi_bcast (hydroshist_fincl1, (max_namlen+2)*size(hydroshist_fincl1), MPI_CHARACTER, 0, mpicom_rof, ier)
    call mpi_bcast (hydroshist_fincl2, (max_namlen+2)*size(hydroshist_fincl2), MPI_CHARACTER, 0, mpicom_rof, ier)
    call mpi_bcast (hydroshist_fincl3, (max_namlen+2)*size(hydroshist_fincl3), MPI_CHARACTER, 0, mpicom_rof, ier)

    call mpi_bcast (hydroshist_avgflag_pertape, size(hydroshist_avgflag_pertape), MPI_CHARACTER, 0, mpicom_rof, ier)
    ...
    if (masterproc) then
       ...
       if (nsrest == nsrStartup .and. finidat_hydros /= ' ') then
          write(iulog,*) '   RTM initial data   = ',trim(finidat_hydros)
       end if
    endif
    ...
    if (trim(hydros_effvel) == 'NULL') then
       effvel_active = .false.
    endif
    ...
    if (hydros_active) then
       if (frivinp_hydros == ' ') then
          call shr_sys_abort( subname//' ERROR: hydros_mode ACTIVE, but frivinp_hydros NOT set' )
       else
          if (masterproc) then
    ...
    if (hydros_tstep <= 0) then
       write(iulog,*) subname,': ERROR hydros step invalid',hydros_tstep
       call shr_sys_abort( subname//' ERROR: hydros_tstep invalid' )
    endif
       
    do i = 1, max_tapes
       if (hydroshist_nhtfrq(i) == 0) then
          hydroshist_mfilt(i) = 1
       else if (hydroshist_nhtfrq(i) < 0) then
          hydroshist_nhtfrq(i) = nint(-hydroshist_nhtfrq(i)*SHR_CONST_CDAY/(24._r8*hydros_tstep))
       endif
    end do
    ...
    if ((nsrest == nsrStartup .and. finidat_hydros /= ' ') .or. &
    ...
    if (nsrest == nsrStartup) then  
       call timemgr_init(dtime_in=hydros_tstep)
    else
    ...
    hydros_trstr = trim(hydros_tracers(1))
    do n = 2,nt_hydros
       hydros_trstr = trim(hydros_trstr)//':'//trim(hydros_tracers(n))
    enddo
    if (masterproc) then
       write(iulog,*)'hydros tracers = ',nt_hydros,trim(hydros_trstr)
    end if
    ...
    call t_startf('hydrosi_grid')

    call getfil(frivinp_hydros, locfn, 0 )
    if (masterproc) then
       write(iulog,*)'Read in RTM file name: ',trim(frivinp_hydros)
    ...
    call t_startf('hydrosi_dec_basins')
    iocn = 0
    nocn = 0
    do nr=1,rtmlon*rtmlat
       ...
       if (abs(gmask(n)) == 1) then    ! land
          ...
          elseif (abs(gmask(n)) == 1) then  ! no ocean outlet, warn user, ignore cell
             write(iulog,*) 'hydrosini ERROR no downstream ocean cell ', &
               g,nr,gmask(nr),dwnstrm_index(nr), &
               n,gmask(n),dwnstrm_index(n)
             call shr_sys_abort()
          else 
             write(iulog,*) 'hydrosini ERROR downstream cell is non-ocean,non-land', &
               g,nr,gmask(nr),dwnstrm_index(nr), &
               n,gmask(n),dwnstrm_index(n)
             call shr_sys_abort()
          endif
    ...
    enddo
    call t_stopf('hydrosi_dec_basins')

    !-------------------------------------------------------
    !--- Now allocate those basins to pes
    !-------------------------------------------------------

    call t_startf('hydrosi_dec_distr')
    ...
    rglo2gdc = 0
    baspe = 0
    maxhydros = int(float(nhydros)/float(npes)*0.445) + 1
    nloops = 3
    minbas = nhydros
    do nl=1,nloops
       maxbas = minbas - 1
       ...
       do nr=1,rtmlon*rtmlat
          if (nocn(nr) > 0 .and. nocn(nr) >= minbas .and. nocn(nr) <= maxbas) then
             ...
             do while (nop(baspe) > maxhydros)
                baspe = baspe + 1
                if (baspe > npes-1) then
                   baspe = 0
                   maxhydros = max(maxhydros*1.5, maxhydros+1.0)   ! 3 loop, .445 and 1.5 chosen carefully
                endif
             enddo
             !--------------
    ...
    if (masterproc) then
       write(iulog,*) 'hydros cells and basins total  = ',nhydros,nbas
       write(iulog,*) 'hydros cells per basin avg/max = ',nhydros/nbas,maxval(nocn)
       write(iulog,*) 'hydros cells per pe    min/max = ',minval(nop),maxval(nop)
    ...
    do n = 0,npes-1
       ...
       num_hydros(n)   = num_hydros(n)   + nop(n)
    ...
    deallocate(pocn)
    call t_stopf('hydrosi_dec_distr')
    ...
    do np = npmin,npmax,npint
       ...
       pid = min(pid,npes-1)
       if (iam == pid) then
          write(iulog,*) 'hydros decomp info',' proc = ',iam, &
             ' begr = ',runoff%begr,&
             ' endr = ',runoff%endr, &
             ' numr = ',runoff%lnumr
    ...
    call t_stopf('hydrosi_print')

    !-------------------------------------------------------
    ! Allocate local flux variables
    !-------------------------------------------------------

    call t_startf('hydrosi_vars')

    allocate (fluxout (begr:endr,nt_hydros), &
              ddist   (begr:endr),        &
              evel    (begr:endr,nt_hydros), &
              sfluxin (begr:endr,nt_hydros), stat=ier)
    ...
    deallocate(rglo2gdc)
    deallocate(rgdc2glo)
    call shr_mpi_sum(lrtmarea,runoff%totarea,mpicom_rof,'hydros totarea',all=.true.)
    if (masterproc) write(iulog,*) 'Rtmini earth area',4.0_r8*shr_const_pi*1.0e6_r8*re*re
    if (masterproc) write(iulog,*) 'Rtmini hydros area  ',runoff%totarea
    ...
    if (dtovermax > 0._r8) then
       delt_hydros_max = (1.0_r8/dtovermax)*cfl_scale
    else
       write(iulog,*) 'hydrosini ERROR in delt_hydros_max ',delt_hydros_max,dtover
       call shr_sys_abort()
    endif
    if (masterproc) write(iulog,*) 'hydros max timestep = ',delt_hydros_max,' (sec) for cfl_scale = ',cfl_scale

    call t_stopf('hydrosi_vars')
...
!EOP
    ...
    real(r8) :: sumfin(nt_hydros),sumfex(nt_hydros)
    real(r8) :: sumrin(nt_hydros),sumdvt(nt_hydros)
    real(r8) :: budget1(nt_hydros)             ! budget check in m3/s
    real(r8) :: budget_global(nt_hydros)       ! global budget sum
    ...
    real(r8) :: delt_hydros                    ! real value of rtm_tstep
...
!-----------------------------------------------------------------------
    ...
    delt_hydros = hydros_tstep*1.0_r8
    if (first_time) then
       delt_save    = delt_hydros
       if (masterproc) write(iulog,*) trim(subname),': hydros act timestep ~ ',delt_hydros
       first_time = .false.
    end if
    ...
    nsub = int(delt_hydros/delt_hydros_max) + 1
    delt = delt_hydros/float(nsub)
    if (delt /= delt_save) then
       if (masterproc) then
          write(iulog,*) trim(subname),': hydros delt update from/to',delt_save,delt,nsub_save,nsub
       end if
    endif
    ...
    if (budget_check) then
       ...
       do nr = runoff%begr,runoff%endr
          ...
          budget1(nt) = budget1(nt) - runoff%volr(nr,nt)/delt_hydros
       enddo
       enddo
       call t_stopf('RTMbudget')
    endif
    ...
    if (budget_check) then
       call t_startf('RTMbudget')
       call shr_mpi_sum(budget1,budget_global,mpicom_rof,'hydros global budget',all=.false.)
       if (masterproc) then
          write(iulog,*) ' '
          abort = .false.
          do nt = 1,nt_hydros
          ...
          if (abort) then
             ...
             call shr_sys_abort(trim(subname)//':hydros budget ERROR')
          endif
       endif
       call t_stopf('RTMbudget')
    endif
  ...
  subroutine RtmFloodInit(frivinp, flakinp, begr, endr, nt_hydros, fthresh, evel, &
    ...
    real(r8), intent(out) :: fthresh( begr: )  ! Rtm water flood threshold
                                               ! [begr:endr]
    real(r8), intent(out) :: evel( begr: , 1: )! effective velocity [begr:endr, nt_hydros] 
    ...
    real(r8) :: effvel4_0(nt_hydros)    ! downstream velocity (m/s)
    real(r8) :: effvel4_5(nt_hydros)    ! downstream velocity (m/s)
    real(r8) :: min_ev4_5(nt_hydros)    ! minimum downstream velocity (m/s)
    ...
    SHR_ASSERT_ALL((ubound(evel)    == (/endr, nt_hydros/)), errMsg(__FILE__, __LINE__))
    ...
    !----------------------
    ! if either is_hydrosflood_on = .true. or is_effvel_on is .true. then do 
    ...
    ! always used when is_hydrosflood_on is .true.).
    !----------------------

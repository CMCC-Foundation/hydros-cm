...
! !USES:
  ...
  use RtmVar      , only : iulog, spval, ispval, nt_hydros
...
! !PUBLIC TYPES:
  ...
  type runoff_flow
     ...
     real(r8), pointer :: rlon(:)         => null() ! hydros longitude list, 1d
     real(r8), pointer :: rlat(:)         => null() ! hydros latitude list, 1d

     real(r8)          :: totarea          ! global area
     integer           :: numr             ! hydros gdc global number of cells
     integer           :: numrl            ! hydros gdc global number of lnd cells
     integer           :: numro            ! hydros gdc global number of ocn cells
...
contains
  ...
  subroutine RunoffInit(begr, endr, numr)
    ...
    allocate(runoff%runoff(begr:endr,nt_hydros),     &
             runoff%dvolrdt(begr:endr,nt_hydros),    &
             runoff%runofflnd(begr:endr,nt_hydros),  &
             runoff%dvolrdtlnd(begr:endr,nt_hydros), &
             runoff%runoffocn(begr:endr,nt_hydros),  &
             runoff%dvolrdtocn(begr:endr,nt_hydros), &
             runoff%area(begr:endr),              &
             runoff%volr(begr:endr,nt_hydros),       &
             runoff%volrlnd(begr:endr,nt_hydros),    &
             runoff%fluxout(begr:endr,nt_hydros),    &
